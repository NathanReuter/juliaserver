...
test('POST /users 201 with name and role', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
...
test('POST /users 201 admin role should not affect', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
...
test('POST /users 200 - invalid role converted to user', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
...
test('POST /users/admin 201 (admin)', async () => {
  const { status } = await request(app())
    .post(apiRoot + '/admin')
    .send({ access_token: adminSession, email: 'd@d.com', password: '123456' })
  expect(status).toBe(201)
...
test('POST /users/admin 401 (public) - Unauthorized user ', async () => {
  const { status } = await request(app())
    .post(apiRoot + '/admin')
    .send({ email: 'd@d.com', password: '123456' })
  expect(status).toBe(401)
...
test('POST /users/admin 401 (user) - Unauthorized user ', async () => {
  const { status } = await request(app())
    .post(apiRoot + '/admin')
...
test('POST /users/admin 201 (admin) - expect role to be admin', async () => {
  const { status, body } = await request(app())
    .post(apiRoot + '/admin')
    .send({ access_token: adminSession, email: 'd@d.com', password: '123456', role: 'admin' })
  expect(status).toBe(201)
  expect(body.user.role).toBe('admin')
})

test('POST /users/admin 400 (admin) - expect 400 for wrong role', async () => {
  const { status } = await request(app())
    .post(apiRoot + '/admin')
    .send({ access_token: adminSession, email: 'd@d.com', password: '123456', role: 'sdadsad' })
  expect(status).toBe(400)
})
