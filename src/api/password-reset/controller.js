...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Julia Server account.<br>
        ...
        &mdash; Julia Server Team
      `
      return sendMail({ toEmail: email, subject: 'Julia Server - Password Reset', content })
    })
